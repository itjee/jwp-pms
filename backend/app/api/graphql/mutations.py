...
from app.api.graphql.types import (
    User, Project, Task, Comment, Event,
    UserInput, ProjectInput, TaskInput, CommentInput, EventInput
)
...
class Mutation:
    ...
    async def register(
        ...
        existing_username = await db.execute(
            select(user_models.User).where(user_models.User.username == user_input.username)
        )
    ...
    async def create_task(
        ...
        # 프로젝트 존재 확인
        project_result = await db.execute(
            select(project_models.Project).where(project_models.Project.id == task_input.project_id)
        )
        project = project_result.scalar_one_or_none()
